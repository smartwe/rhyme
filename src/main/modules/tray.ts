...
class TrayModule extends ModuleWindow {
  ...
  constructor(dirname: string, window: Electron.BrowserWindow) {
    ...
    const logosPath = path.join(__dirname, 'icons');

    const trayIcons = {
      tray: nativeImage.createFromPath(path.join(logosPath, 'png', '32x32.png')).resize({ width: 24, height: 24 }),
      'tray-win32': nativeImage.createFromPath(path.join(logosPath, 'icon.ico')),
    };
